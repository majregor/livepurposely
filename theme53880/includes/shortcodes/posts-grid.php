		...
		switch ($link) {
			...
			foreach ( $posts as $j => $post ) {
				...
				$output .= '<li class="'. $spans .' list-item-'.$count.'">';
					...
					$output .= '<div class="desc"><div class="desc-in">';
						...
						if($excerpt_count >= 1 && $excerpt_count < 55){
							$output .= '<p class="excerpt">';
								$output .= wp_trim_words($excerpt,$excerpt_count);
							$output .= '</p>';
						} else if ($excerpt_count >= 55) {
							$output .= '<p class="excerpt">';
								$output .= wp_trim_words($big_content,$excerpt_count);
							$output .= '</p>';
						}

						if ($meta == 'yes' && $custom_class == 'last-from-blog') {
							// begin post meta
							$output .= '<div class="post_meta author">';
								// post author
								$output .= '<div class="post_author_big">';
									$output .= '<h3><span class="title">'.__("Written By", CURRENT_THEME).'</span></h3>';									

									$output .= '<p class="post-author_gravatar">';
										if(function_exists('get_avatar')) { $output .= get_avatar( get_the_author_meta( 'user_email' ), $size = '65' ); }  
									$output .= '</p>';

									$output .= '<div class="desc-author">';
										$output .= '<a href="'.get_author_posts_url(get_the_author_meta( 'ID' )).'">'.get_the_author_meta('display_name').'</a>';
										if (get_the_author_meta( 'description' )) {
											$output .= '<div class="description">'.get_the_author_meta( 'description' ).'</div>';
										}
									$output .= '</div>';
